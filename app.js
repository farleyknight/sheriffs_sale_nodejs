...
var haversineSQL = function(latitude, longitude, lat_attr, lng_attr) {
...
var distanceWhere = function(distance, lat, lng, lat_attr, lng_attr) {
  return haversineSQL(lat, lng, lat_attr, lng_attr)
    + " BETWEEN 0 and " + distance;
};

var distanceSelect = function(latitude, longitude) {
  var sql =
    haversineSQL(latitude, longitude, 'properties.latitude', 'properties.longitude')
    + " as distance";
...
app.get('/properties.json', function(req, res) {
  ...
  var query = Property.findAll({
    attributes: ['*', distanceSelect(lat, lng)],

    where: Sequelize.and(
      ['price_in_dollars < ?', max_price * 1000],
      ['price_in_dollars > ?', min_price * 1000],
      distanceWhere(20, lat, lng, 'latitude', 'longitude')
    ),
